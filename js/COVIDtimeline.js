...
function makeTimeline(fullSweeps) {
  ...
  for (var i = 0; i < fullSweeps.length; i++) {
    let d = getJsDateFromExcel(
      fullSweeps[i]["Start date"],
      fullSweeps[i]["End date"]
    );
    let thisTitle = fullSweeps[i]["Month"];
    //d = d.getTime();
    if (titles.indexOf(thisTitle) == -1) {
...
function makeCircles(sweeps) {
  ...
  if (svgSupport) {
    animationElement =
      '<object type="image/svg+xml" class="svgTree" style="visibility:hidden;width:40px;height:55px;" data="img/svgTree.svg"></object>';
  }

  if (first == labelDates[0]) {
    firstLabel = true;
    $("#line").append(
      '<div class="labelLine left above" id="label0" style="left:0%;"><div class="popupSpan label">' +
        labelTitles[0] +
        '</div><div class="popupSpan square"></div></div>'
    );
  } else {
    $("#line").append(
      '<div class="circle labelled left" id="circle0" style="left:0%;"><div class="popupSpan label">' +
        sweeps[dates[0]] +
  ...
  var currentData = fullSweeps.filter(function (sweep) {
    return sweep.Title == sweeps[dates[0]];
  });
  $("#dataTable").DataTable({
    data: currentData,
    autowidth: true,
    ...
    columns: [
      ...
      },
      {
        ...
        render: function (value) {
          return formatMonthYear(getJsDateFromExcel(value));
        },
      },
      {
        title: "End date",
      ...
      {
        title: "Mental health measures used",
        data: "Measures",
        className: "none",
  ...
  /*   $("#dataTable")
    ...
    .on("responsive-display", function (e, datatable, row, showHide, update) {
      if (showHide) {
        ...
        $("li[data-dt-row='" + row.index() + "'] .dtr-data").each(function () {
          if ($(this).text() == "") {
            $(this).parent().hide();
          }
        });
  ...
  $("#line").append(
    '<div class="circle labelled right" id="circle' +
      (dates.length - 1) +
      '" style="right:0;"><div class="popupSpan label">' +
      sweeps[dates[dates.length - 1]] +
      '</div><div class="popupSpan tree">' +
      animationElement +
      "</div></div>"
  );
    ...
    .on("search.dt", function () {
      ...
      if (searchField.length >= 3) {
        ...
        for (var i in searchTerms) {
          if (searchTerms[i].length < minLength) {
        ...
        $.each(fullSweeps, function (key, val) {
          ...
          if (andRegex.test(s)) {
            ...
            if (
              matches.indexOf(
                dates.indexOf(getKeyByValue(sweeps, val["Month"]))
              ) == -1
            ) {
  ...
  if (dates.length < 2) {
    $("#timeline .sweepTitle")
      .html(
        "Showing measures used in sweep: <b>" +
          sweeps[dates[0]] +
          "</b><br><i>No other sweeps</i>"
      )
      .addClass("mb-4");
    $("#line").hide();
  }
...
function selectDate(selector) {
  ...
  $("#timeline .sweepTitle").html(
    "Showing studies that started data collection in: <b>" +
      sweeps[dates[currentIndex]] +
      "</b><br><i>click on the timeline to see other timepoints"
  );
  var currentData = fullSweeps.filter(function (sweep) {
    return sweep["Month"] == currentLabel;
  });
  $("#dataTable").DataTable().clear();
  $("#dataTable").DataTable().rows.add(currentData).draw();
  $("#dataTable").DataTable().responsive.recalc().columns.adjust().draw();
  $("#dataTable").dataTable().css("width", "100%");

  if (
    currentIndex > 0 &&
    collide($("#" + selector + " .label")[0], $("#circle0 .label")[0])
  ) {
