...
$.when(
    ...
    $(data)
      ...
      .each(function (e) {
        ...
        if (Name.indexOf("Short-Form") > -1) {
          standardInstruments[Name.replace(/Short-Form/g, "Short Form")] =
        ...
        $(this)
          ...
          .each(function () {
            ...
            if (
              $(this)[0].tagName == "HR" ||
              parasInToolTip > 3 ||
              $(this).text().indexOf("Reference") > -1 ||
              ($(this).text().endsWith(":") && parasInToolTip == 3)
            )
              return false;
  ...
  $.getJSON(
    "api/getSweeps.php?studyid=" + $("#studyid").val(),
    function (data) {
      fullSweeps = data;
    }
  )
...
function makeTimeLineGroups(fullSweeps) {
  ...
  for (var i = 0; i < tlGroupTitles.length; i++) {
    let thisTlGroupStart = fullSweeps
      .filter(function (sweep) {
    ...
    tlGroups = tlGroups.sort(function (a, b) {
      if (a.tlDate > b.tlDate) {
        return -1;
      }
      if (b.tlDate > a.tlDate) {
        return 1;
      }
    });
  }
...
function makeTimeline(fullSweeps, timelineGroup) {
  ...
  if (timelineGroup == null) {
    updateTimelineGroup = true;
  }
  //default to showing the first group if none is specified.
  if (
    fullSweeps[0].hasOwnProperty("Timeline Group") &&
    fullSweeps[0]["Timeline Group"] !== null
  ) {
    timelineGroup = timelineGroup || fullSweeps[0]["Timeline Group"];
    if (updateTimelineGroup) {
      makeCircles(tlGroups, $("#groupLine"));
    }
    thisGroupSweeps = fullSweeps.filter(function (sweep) {
      return sweep["Timeline Group"] == timelineGroup;
    });
  } else {
    $("#lineContGroup").remove();
  }
  sweeps = [];
  let sweepTitles = thisGroupSweeps
    .map(function (sweep) {
      return sweep["Label"];
    })
    .filter(function (item, pos, self) {
  ...
  for (var i = 0; i < sweepTitles.length; i++) {
    ...
    let sweep = thisGroupSweeps.filter(function (sweep) {
      return sweep["Label"] == sweepTitles[i];
    })[0];
    sweeps.push(
      new tlEntry(
...
function makeCircles(tlEntries, $line) {
  ...
  for (i = 0; i < tlEntries.length; i++) {
    ...
    if (svgSupport) {
      animationElement =
        '<object type="image/svg+xml" class="svgTree" style="visibility:hidden;width:40px;height:52px;" data="img/svgTree.svg"></object>';
    }

    //Draw the date circle
    if (i == 0) {
      $line.append(
        '<div class="circle labelled left" title="' +
          formatLabels(tlEntries[i].label, "tooltip") +
          '" data-id="circle' +
          i +
          '" style="left: 0;"><div class="popupSpan label">' +
      ...
      $line.append(
        '<div class="circle labelled right" title="' +
          formatLabels(tlEntries[i].label, "tooltip") +
          '" data-id="circle' +
          i +
          '" style="right: 0;"><div class="popupSpan label">' +
          formatLabels(tlEntries[i].label, "tl") +
          '</div><div class="popupSpan tree">' +
          animationElement +
          "</div></div>"
    ...
    if (svgSupport) {
      var elem = $(
        "#" + $line.attr("id") + ' .circle[data-id="circle' + i + '"] .svgTree'
      );
      $(elem)[0].addEventListener("load", function () {
        ...
        if ($(this).parent().hasClass("treeShown")) {
          animateTreeAnimation(
            $(this).closest(".line").attr("id"),
            $(this).closest(".circle").attr("data-id").replace("circle", ""),
            "grow"
          );
        }
      });
...
function showTrees() {
  ...
  if (searchField.length >= minSearchLength) {
    ...
    for (var i in searchTerms) {
      if (searchTerms[i].length < minSearchLength) {
  ...
  for (i = 0; i < tlGroups.length; i++) {
    ...
    if (elem.length) {
      if (groupMatches.indexOf(i) == -1 && $(elem).hasClass("treeShown")) {
        ...
        groupMatches.indexOf(i) != -1 &&
        !$(elem).hasClass("treeShown")
      ) {
...
function displaySweepInfo(tlEntry, suffix) {
  ...
  if (tlEntry.physHealth) {
    ...
    $("#sweepPhysHealth > span").html(
      "No detailed physical health assessment matching Catalogue criteria"
    );
  }
  ...
  if (dates[0].getFullYear() == dates[1].getFullYear()) {
    if (dates[0].getMonth() == dates[1].getMonth()) {
      ...
      dateStr = monthNames[dates[0].getMonth()] + " " + dates[0].getFullYear();
    } else {
...
function selectDate($circle, report) {
  ...
  if ($line.attr("id") == "groupLine") {
    ...
    var currentData = fullSweeps.filter(function (sweep) {
      return sweep.Title == currentSweepTitle;
    });
  ...
  if (report) {
    if (tlGroupSelection) {
      gtag("event", "click", {
        event_label:
          "Study: " + $("#studyid").val() + " Group: " + currentSweepTitle,
      ...
      gtag("event", "click", {
        event_label:
          "Study: " + $("#studyid").val() + " Sweep: " + currentSweepTitle,
...
function deCollide(currentIndex, $circle, $line) {
  ...
  if (
    currentIndex > 0 &&
    !contained($circle.find(".label")[0], $line.parent(".lineCont")[0])
  ) {
  ...
  if (
    currentIndex > 0 &&
    collide($circle.find(".label")[0], $circle0.find(".label")[0])
  ) {
  ...
  if (
    currentIndex < circleCount - 1 &&
    collide(
      $circle.find(".label")[0],
      $line.find(".circle.right").find(".label")[0]
    )
  ) {
...
function animateTreeAnimation(lineID, index, direction) {
  if (svgSupport) {
    elem = $(
      "#" + lineID + ' .circle[data-id="circle' + index + '"] .svgTree'
    )[0];
