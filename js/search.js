...
searchSwaps["Emotional problems"] = [
  "Anxiety",
  "Phobia",
  "Agoraphobia",
  "Panic",
...
searchSwaps["Behavioural problems"] = [
  "Externalising",
  "Externalizing",
  "Conduct disorder",
  "Conduct problems",
...
searchSwaps["Psychological Distress"] = [
  "Depression",
  "Anxiety",
  "Stress",
  "Dysthymia",
...
searchSwaps["Borderline personality disorder"] = [
  "Borderline",
  "Emotionally unstable personality disorder",
  "EUPD",
  "BPD",
];
searchSwaps["Borderline"] = [
  "Borderline personality disorder",
  "Emotionally unstable personality disorder",
  "EUPD",
  "BPD",
];
searchSwaps["Bipolar disorder"] = [
  "Mania",
  "Manic depression",
...
searchSwaps["Eating disorders"] = [
  "Anorexia",
  "Bulimia",
  "Binge eating",
  "Feeding",
];
searchSwaps["Antisocial personality disorder"] = [
...
searchSwaps["Antisocial behaviour"] = [
...
searchSwaps["Schizotypal personality disorder"] = [
  "Schizophrenia",
  "Psychosis",
  "Hallucination",
  "Delusion",
...
searchSwaps["Psychological wellbeing"] = [
  "Well being",
  "Wellness",
  "Well-being",
  "Happiness",
...
searchSwaps["Treatment and service use"] = [
  "Patient",
  "Hospital",
  "Psychiatry",
  "Sectioned",
...
searchSwaps["Treatment - medication"] = [
  "Antidepressant",
  "Antipsychotic",
  "Pharmacology",
  "Prescription",
...
searchSwaps["Treatment - in-patient"] = [
  "Hospital",
  "Involuntary",
  "Section",
  "sectioned",
...
searchSwaps["Treatment - specialist"] = [
  "Psychotherapy",
  "Psychology",
  "Psychiatry",
  "Talking therapy",
...
searchSwaps["Treatment and service use - school support"] = [
  "SEN",
  "IEP",
  "Teacher[’']?s aid",
  "Special school",
...
$.getJSON("api/getSearchData.php", function (data) {
  ...
  $(document).ready(function () {
    createFilters();
    $("#Covid-19_data_collection_during_pandemic")
      .parent()
      .addClass("mb-2")
      .addClass("font-weight-bold")
      .insertAfter($("#relatedAndOr").parent());
    $('[data-toggle="tooltip"]').tooltip({
      container: "body",
      boundary: "window",
      offset: 0,
      customClass: "searchInfobuttonTooltip",
    });
    updateFilters(true);
...
function parseNumbers(
  ...
  for (i = 0; i < numbers.length; i++) {
    ...
    if (assumeDates && numbers[i] < 100) {
      numbers[i] = 2000 + Number(numbers[i]);
    }
  }
...
function createFilters() {
  ...
  $.each(searchData, function (key, val) {
    if (val["Related themes"])
      relatedThemes = relatedThemes.concat(val["Related themes"].split(","));
    if (val["Study design"])
      studyDesign = studyDesign.concat(val["Study design"].split(","));
    if (val["Sample characteristics"])
      sampleCharacteristics = sampleCharacteristics.concat(
        val["Sample characteristics"].split(",")
      );
    if (val["Geographic coverage"])
      geographicCoverage = geographicCoverage.concat(
        val["Geographic coverage"].split(",")
      );
    if (val["Complementary data"])
      complementaryData = complementaryData.concat(
        val["Complementary data"].split(",")
      );
    let start = parseNumbers(String(val["Start date"]), true);
    if (start !== null) {
      starts = starts.concat(start);
    }
    let age = parseNumbers(
      String(val["Age at recruitment"]),
      false,
      false,
      true
    );
  ...
  for (var i = 1; i < relatedThemes.length; i++) {
    ...
    relatedThemes[i] =
      relatedThemes[i].charAt(0).toUpperCase() + relatedThemes[i].substr(1);
  }
  for (var i = 1; i < studyDesign.length; i++) {
    studyDesign[i] = studyDesign[i].trim();
    studyDesign[i] =
      studyDesign[i].charAt(0).toUpperCase() + studyDesign[i].substr(1);
  }
  for (var i = 1; i < sampleCharacteristics.length; i++) {
    sampleCharacteristics[i] = sampleCharacteristics[i].trim();
    sampleCharacteristics[i] =
      sampleCharacteristics[i].charAt(0).toUpperCase() +
      sampleCharacteristics[i].substr(1);
  }
  for (var i = 1; i < geographicCoverage.length; i++) {
    geographicCoverage[i] = geographicCoverage[i].trim();
    geographicCoverage[i] =
      geographicCoverage[i].charAt(0).toUpperCase() +
      geographicCoverage[i].substr(1);
  }
  for (var i = 1; i < complementaryData.length; i++) {
    complementaryData[i] = complementaryData[i].trim();
    complementaryData[i] =
      complementaryData[i].charAt(0).toUpperCase() +
      complementaryData[i].substr(1);
  }
  ...
  geographicCoverage.unshift(
    geographicCoverage.splice(geographicCoverage.indexOf("National"), 1)[0]
  );
  geographicCoverage.unshift(
    geographicCoverage.splice(geographicCoverage.indexOf("Local area"), 1)[0]
  );
  ...
  $.each(relatedThemes, function (index, value) {
    ...
    var checkbox =
      "<div class='custom-control custom-switch'><input type='checkbox' class='relatedThemesFilter toggleFilter form-control custom-control-input' id='" +
      idValue +
      "' value='" +
      value +
      "' name='" +
  ...
  $.each(studyDesign, function (index, value) {
    ...
    var checkbox =
      "<div class='custom-control custom-switch'><input type='checkbox' class='studyDesignFilter toggleFilter form-control custom-control-input' id='" +
      idValue +
      "' value='" +
      value +
      "' name='" +
  ...
  $.each(sampleCharacteristics, function (index, value) {
    ...
    var checkbox =
      "<div class='custom-control custom-switch'><input type='checkbox' class='sampleCharacteristicsFilter toggleFilter form-control custom-control-input' id='" +
      idValue +
      "' value='" +
      value +
      "' name='" +
  ...
  $.each(geographicCoverage, function (index, value) {
    ...
    var checkbox =
      "<div class='custom-control custom-switch'><input type='checkbox' class='geographicCoverageFilter toggleFilter form-control custom-control-input' id='" +
      idValue +
      "' value='" +
      value +
      "' name='" +
  ...
  $.each(complementaryData, function (index, value) {
    ...
    var checkbox =
      "<div class='custom-control custom-switch'><input type='checkbox' class='complementaryDataFilter toggleFilter form-control custom-control-input' id='" +
      idValue +
      "' value='" +
      value +
      "' name='" +
  ...
  starts = uniqueArray(starts).sort(function (a, b) {
    return a - b;
  });
  ages = uniqueArray(ages).sort(function (a, b) {
    return a - b;
  });
  sizes = uniqueArray(sizes).sort(function (a, b) {
    return a - b;
  });
  $("#ageSlider").before(
    "<div><b style='float:left;'>" +
      (ages[0] == 0 ? "Birth" : ages[0]) +
      "</b><b style='float:right;'>" +
      ages[ages.length - 1] +
      "</b></div>"
  );
  ...
  $("#sizeSlider").before(
    "<div><b style='float:left;'>" +
      numberWithCommas(sizes[0]) +
      "</b><b style='float:right;'>" +
      numberWithCommas(sizes[sizes.length - 1]) +
      "</b></div>"
  );
  sizeSlider = $("#sizeSlider").slider({
    ...
    formatter: function (value) {
      if (value) {
        var values = String(value).split(",");
        return (
          numberWithCommas(values[0]) + " - " + numberWithCommas(values[1])
        );
      }
    },
  });
...
function clearFilters() {
  ...
  dateSlider.slider(
    "setValue",
    new Array(
      Number($("#dateRangeFilter .slider-handle").attr("aria-valuemin")),
  ...
  ageSlider.slider(
    "setValue",
    new Array(
      Number($("#ageRangeFilter .slider-handle").attr("aria-valuemin")),
  ...
  sizeSlider.slider(
    "setValue",
    new Array(
      Number($("#sizeRangeFilter .slider-handle").attr("aria-valuemin")),
      Number($("#sizeRangeFilter .slider-handle").attr("aria-valuemax"))
    )
  );
  $('.toggleFilter:not(".andOr"):checked').each(function () {
    $(this).prop("checked", false);
  });
  $('#dateSlider, #ageSlider, #sizeSlider, .toggleFilter:not(".andOr")').change(
    function () {
      updateFilters();
    }
  );
...
function updateFilters(firstRun) {
  ...
  if (
    ...
    $("#filtersToggle").is(":visible")
  ) {
    updateSearchResults(originalSearchData);
    return true;
  }
  ...
  if ($(".relatedThemesFilter:checked").length) {
    $(".relatedThemesFilter:checked").each(function () {
      relatedThemesFilters.push(this.name);
    });
  }
  if ($(".studyDesignFilter:checked").length) {
    $(".studyDesignFilter:checked").each(function () {
      studyDesignFilters.push(this.name);
    });
  }
  if ($(".sampleCharacteristicsFilter:checked").length) {
    $(".sampleCharacteristicsFilter:checked").each(function () {
      sampleCharacteristicsFilters.push(this.name);
    });
  }
  if ($(".geographicCoverageFilter:checked").length) {
    $(".geographicCoverageFilter:checked").each(function () {
      geographicCoverageFilters.push(this.name);
    });
  }
  if ($(".complementaryDataFilter:checked").length) {
    $(".complementaryDataFilter:checked").each(function () {
      complementaryDataFilters.push(this.name);
    });
  }
  ...
  if (firstRun) {
    ...
    var sampleCharacteristicsState = localStorage.getItem(
      "sampleCharacteristicsState"
    );
    var geographicCoverageState = localStorage.getItem(
    ...
    try {
      ...
      if (relatedThemesState) {
        ...
        for (i = 0; i < relatedThemesFilters.length; i++) {
          $(
            ".relatedThemesFilter[name='" + relatedThemesFilters[i] + "']"
          ).prop("checked", true);
        }
      }
      if (studyDesignState) {
        ...
        for (i = 0; i < studyDesignFilters.length; i++) {
          $(".studyDesignFilter[name='" + studyDesignFilters[i] + "']").prop(
            "checked",
            true
          );
        }
      ...
      if (sampleCharacteristicsState) {
        ...
        $("#scAndOr").prop(
          "checked",
          sampleCharacteristicsFilters[0] == "true"
        );
        sampleCharacteristicsFilters.shift();
        for (i = 0; i < sampleCharacteristicsFilters.length; i++) {
          $(
            ".sampleCharacteristicsFilter[name='" +
              sampleCharacteristicsFilters[i] +
              "']"
          ).prop("checked", true);
        }
      }
      if (geographicCoverageState) {
        ...
        for (i = 0; i < geographicCoverageFilters.length; i++) {
          $(
            ".geographicCoverageFilter[name='" +
              geographicCoverageFilters[i] +
              "']"
          ).prop("checked", true);
        }
      }
      if (complementaryDataState) {
        ...
        for (i = 0; i < complementaryDataFilters.length; i++) {
          $(
            ".complementaryDataFilter[name='" +
              complementaryDataFilters[i] +
              "']"
          ).prop("checked", true);
        }
      }
      //Attach Filter listeners
      $("#dateSlider, #ageSlider, #sizeSlider, .toggleFilter")
        .off("change")
        .change(function () {
          updateFilters();
        });
  ...
  if (relatedThemesFilters.length) {
    ...
    localStorage.setItem(
      "relatedThemesState",
      [$("#relatedAndOr").prop("checked")].concat(
        Array(relatedThemesFilters.join(","))
      )
    );
  } else {
    localStorage.removeItem("relatedThemesState");
  ...
  if (studyDesignFilters.length) {
    ...
    localStorage.setItem(
      "studyDesignState",
      [$("#sdAndOr").prop("checked")].concat(
        Array(studyDesignFilters.join(","))
      )
    );
  } else {
    localStorage.removeItem("studyDesignState");
  ...
  if (sampleCharacteristicsFilters.length) {
    ...
    localStorage.setItem(
      "sampleCharacteristicsState",
      [$("#scAndOr").prop("checked")].concat(
        Array(sampleCharacteristicsFilters.join(","))
      )
    );
  } else {
    localStorage.removeItem("sampleCharacteristicsState");
  ...
  if (geographicCoverageFilters.length) {
    ...
    localStorage.setItem(
      "geographicCoverageState",
      [$("#gcAndOr").prop("checked")].concat(
        Array(geographicCoverageFilters.join(","))
      )
    );
  } else {
    localStorage.removeItem("geographicCoverageState");
  ...
  if (complementaryDataFilters.length) {
    ...
    localStorage.setItem(
      ...
      [$("#cdAndOr").prop("checked")].concat(
        Array(complementaryDataFilters.join(","))
      )
    );
  } else {
    localStorage.removeItem("complementaryDataState");
  ...
  if (
    $("#dateRangeFilter .slider-handle").attr("aria-valuemin") ==
      dateRange[0] &&
    ...
    filtersActive = true;
    filteredData = filterItems(filteredData, function (study) {
      return rangeOverlaps(parseNumbers(study["Start date"], true), dateRange);
    });
    localStorage.setItem("dateSliderState", dateRange.join(","));
  }
  if (
    $("#ageRangeFilter .slider-handle").attr("aria-valuemin") == ageRange[0] &&
    ...
    filteredData = filterItems(filteredData, function (study) {
      return rangeOverlaps(
        parseNumbers(study["Age at recruitment"], false, false, true),
        ageRange
      );
    });
  ...
  if (
    $("#sizeRangeFilter .slider-handle").attr("aria-valuemin") ==
      sizeRange[0] &&
    ...
    filteredData = filterItems(filteredData, function (study) {
      return rangeOverlaps(
        [
          parseNumbers(
            String(study["Sample size at recruitment"]).replaceAll(",", ""),
            false,
            true
          ),
        ],
...
function updateSearchResults(newSearchData, noAdditions) {
  ...
  if (
    newSearchData &&
    Object.keys(newSearchData).toString() != Object.keys(searchData).toString()
  ) {
  ...
  if ($("#studyList").html() != output) {
    $("#studyList").html(output);
  }
  var searchSummary = "";
  if (!noAdditions && additions.length) {
    searchSummary +=
      "Adding the alternative terms: <i>" +
      additions.join(", ") +
      "</i> based on your search terms <a href='#' onclick='updateSearchResults(false, true);' >UNDO</a><br>";
  }
    ...
    if (typeof gtag == "function") {
      ...
      gtag("event", ID, {
        event_label:
          ...
          $("#searchField").val() +
          " RelThemes: " +
          localStorage.getItem("relatedThemesState") +
          " Date: " +
          localStorage.getItem("dateSliderState") +
          " Age: " +
          localStorage.getItem("ageSliderState") +
          " Size: " +
          localStorage.getItem("sizeSliderState"),
